...
export function attachWebRtcSignaling(server, { logger = console } = {}) {
  ...
  // Phase 40: cache the last logged renderMode so we only emit a log line
  // when the SSR tab's effective render mode CHANGES (e.g. on GL context
  // loss or recovery). Steady-state "gl" is silent.
  let lastLoggedRenderMode = null;
    ...
    async function dispatchMessage(msg) {
      ...
      if (msg.type === "ssr-stats" && conn.role === "ssr-tab") {
        ...
        if (incoming && typeof incoming === "object" && !Array.isArray(incoming)) {
          ...
          // Log SSR effective render mode only when it changes (GL context
          // loss/recovery). Steady-state "gl" stays silent so the log isn't
          // flooded with identical heartbeats.
          const rm = limited?.renderMode;
          if (typeof rm === "string" && rm.length > 0 && rm !== lastLoggedRenderMode) {
            logger.info(`[ssr-stats] renderMode=${rm} (was ${lastLoggedRenderMode ?? "(none)"})`);
            lastLoggedRenderMode = rm;
          }
        }
        return;
      }
