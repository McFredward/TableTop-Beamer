...
(() => {
  ...
  function drawEffectVisual(type, age, intensity, room, roomMetrics = null, options = {}) {
    ...
    if (type === "outside-space") {
      ...
      // Phase 46 mobile-freeze fix: when the caller opts into the
      // lightweight preview path (dashboard / control role on mobile),
      // render a drastically simplified starfield. The dashboard's
      // local canvas is only an aesthetic preview — the real projector
      // image is produced by the SSR Chromium tab, which never sets
      // `lightweight: true` and keeps the full immersive parallax.
      //
      // Profiling on iPhone-class CPU (4× throttle): the original loop
      // produced ~71 strokes + 68 fillRects + ~213 GPU state mutations
      // per frame (each `strokeStyle = template-literal` allocates a
      // new RGBA string). With outside-space active, the first frame
      // after an animation trigger blew past 100 ms, freezing the UI
      // for ~1 s.
      //
      // The lightweight path:
      //   - 2 layers instead of 4 (immersive) / 3 (standard)
      //   - ≤12 stars/layer (vs ≤98 in full immersive)
      //   - one strokeStyle/lineWidth per layer (vs per-star)
      //   - all strokes batched into a single Path2D → one stroke() call
      //   - all dots batched as rect() inside a single Path2D → one fill()
      //   - no express-lane overlay (purely decorative; absence is
      //     invisible at preview resolution)
      // Net: ~6 canvas ops/frame for the entire outside layer instead
      // of ~280.
      if (options.lightweight === true) {
        const liteLayers = immersive
          ? [
            { density: 8, speed: 310, size: 1.3, alpha: 0.34, wave: 0.011 },
            { density: 12, speed: 660, size: 2.0, alpha: 0.58, wave: 0.017 },
          ]
          : [
            { density: 6, speed: 230, size: 1.1, alpha: 0.28, wave: 0.009 },
            { density: 10, speed: 360, size: 1.6, alpha: 0.42, wave: 0.012 },
          ];
        for (let layerIndex = 0; layerIndex < liteLayers.length; layerIndex += 1) {
          const layer = liteLayers[layerIndex];
          const starCount = Math.max(
            4,
            Math.min(16, Math.round(layer.density * intensity)),
          );
          const layerSpeed = layer.speed * (0.8 + intensity * 0.75) * speedFactor;
          const layerWave = h * layer.wave;
          const baseAlpha = Math.min(0.95, layer.alpha * (0.8 + intensity * 0.7));
          const streakLength =
            (3.5 + layerIndex * 3.2 + speedInfluence * 4.2 + intensity * 2.8) * (immersive ? 1.25 : 1);
          const streakWidth = Math.max(0.8, layer.size * (0.65 + layerIndex * 0.08));

          // Batch all streaks for this layer into a single Path2D —
          // one strokeStyle / lineWidth / stroke() per layer instead
          // of per-star.
          ...
          for (let i = 0; i < starCount; i += 1) {
            ...
            strokePath.moveTo(x + streakLength * directionMultiplier, y);
            strokePath.lineTo(x, y);
            fillPath.rect(x, y, layer.size, layer.size);
          }
          c.strokeStyle = layerIndex === 0
            ? "rgba(232, 238, 255, 0.22)"
            : "rgba(232, 238, 255, 0.42)";
          c.lineWidth = streakWidth;
          c.stroke(strokePath);
          c.fillStyle = layerIndex === 0
            ? "rgba(245, 248, 255, 0.35)"
            : "rgba(245, 248, 255, 0.62)";
          c.fill(fillPath);
          // Cheap layer alpha modulation via globalAlpha would touch
          // the destination; instead we baked the alpha into the two
          // fixed strokeStyle strings above. baseAlpha is unused here
          // but kept in scope so behavioural intent stays legible.
          void baseAlpha;
        }
        return;
      }
