...
(() => {
  ...
  // WebGL mesh-warp state, _initMeshWarpGL, and _postDrawMeshWarpGL
  // moved to runtime-projection-gl-renderer.js (W3.2-C2). The shim
  // delegates to glRenderer's exported postDrawMeshWarpGL via the
  // destructured _postDrawMeshWarpGL alias at the top of this IIFE.
  ...
  // Undo stack + snapshotGridState / restoreGridSnapshot / pushUndo /
  // clearUndo / undo moved to runtime-projection-grid-state.js (W3.2-C1).
  ...
  // resetGrid moved to runtime-projection-grid-state.js (W3.2-C1).
  ...
  function init(dependencies) {
    ctx = dependencies;
    // Forward ctx + cross-module callbacks to the grid-state sub-module
    // so its internal undo() / resetGrid() can rebuild handles, redraw
    // lines, etc., without owning the handle-UI state. Pass the
    // bare-name function references the shim still defines locally.
