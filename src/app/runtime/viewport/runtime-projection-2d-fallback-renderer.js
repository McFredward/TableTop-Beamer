...
(() => {
  ...
  function postDrawMeshWarp2D(canvas, canvasCtx) {
    ...
    //
    // 2026-05-14 streifen-fix: on /output/ (SSR Chromium → projector), pre-
    // composite the snapshot onto OPAQUE BLACK before the per-cell warp.
    //
    // Background: when WebGL fails to initialise on this hardware class
    // (Intel iGPU + Mesa under Xvfb + Chromium GPU-blocklist), the
    // renderer falls through to THIS 2D path. The Phase 30 h7 INFLATE=4
    // overlap was designed to hide rasterizer-edge halos: each cell's
    // clip polygon is dilated 4 px outward so adjacent cells' INFLATEd
    // footprints overlap by ~8 px. With OPAQUE source content that
    // overlap is invisible (identical content drawn twice = same color).
    // But the cluster / solid-color overlay defaults emit alpha ≈ 0.35;
    // with source-over compositing two passes accumulate alpha to
    // 1-(1-α)² ≈ 0.58 inside the overlap, producing visible LIGHTER
    // bands at every cell boundary AND inside-cell triangle diagonal —
    // operator's recurring streifen.
    //
    // Pre-compositing the snapshot onto opaque black flattens all alpha
    // into RGB before the warp loop sees it. Subsequent INFLATE-overlap
    // draws then redraw the same opaque-RGB content twice (identical
    // bytes — alpha stays at 1.0 throughout), so the streifen disappear.
    //
    // /output/ specifically: the projector's "no light" pixel is opaque
    // black, exactly what we're filling the transparent regions with,
    // so this is a no-op for the visible result outside the room
    // polygons. Mirrors the GL renderer's _gl.clearColor(0,0,0,1) on
    // /output/.
    //
    // Dashboard explicitly KEEPS the historic transparent-snapshot
    // behavior so the projection-mapping editor's overlay handles
    // composite correctly over the fx-canvas content beneath.
    const isOutput = (typeof document !== "undefined")
      && document.body?.dataset?.outputRole === "final-output";
    _warpTmpCtx.setTransform(1, 0, 0, 1, 0, 0);
    if (isOutput) {
      _warpTmpCtx.fillStyle = "#000";
      _warpTmpCtx.fillRect(0, 0, w, h);
    } else {
