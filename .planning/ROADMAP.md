...
2. M2 HF2 Lifecycle Closure: rehydrate/no-replay expired one-shot semantics und low-end hardening sind PASS.
3. M3 Coordinate Determinism Closure: overlay mapping bleibt stabil ueber browser/DPR/fullscreen transitions.
4. M4 Mixed-Media Final-Output Closure: room `mp4` start (`malfunction`) stoppt room GIF rendering nicht mehr auf `/output/final`.
...
- `/output/final` mixed-media lifecycle bleibt stabil; room GIF rendering bleibt nach room `mp4` (`malfunction`) Start sichtbar.
...
- Plan 9-HF3 ist abgeschlossen; die P0-Hotfix-Gates fuer Mapping/Mixed-Media/Weak-Hardware/Feedback sind geschlossen.

HF3 Closure (runtime testing package):
- Cross-browser/dpr/fullscreen coordinate mapping determinism for polygon overlays is a hard gate.
- `/output/final` mixed-media lifecycle bug (`malfunction` mp4 suppresses GIF room rendering) is a hard gate.
...
20. M0 HF6 Area-Drop Repro Closure: deterministische RED-Repro zeigt `Bunker`-Drop auf Firefox/mobile-class gegen Chrome-Baseline.
21. M0 HF6 Merge-Lineage Closure: source-merge Diagnostik (`saved/default/imported`) ist ausfuehrbar und identifiziert first-drop Ursachen.
22. M0 HF6 Fallback-Guard Closure: default area ersetzt keine validen multi-area subset Daten mehr.
...
30. M0 HF7 FAIL->PASS Closure: identischer HF7-Testsatz ist pre-fix FAIL und post-fix PASS dokumentiert (historische Baseline).
31. M0 HF8 All-Board Canonical Load Closure: alle Boards laden/applyen play-areas deterministisch aus kanonischen Saved-Sources.
32. M0 HF8 Defaults-Reapply Closure: `Load global defaults` stellt board-spezifische play-areas deterministisch wieder her.
33. M0 HF8 Explicit Error Visibility Closure: canonical load/apply failures sind explizit operator-sichtbar (toast/status mit Kontext), kein silent fallback masking.
34. M0 HF8 All-Board Parity Closure: control-view und `/output/final` bleiben identisch in canonical set + `areaCount` + `areaIdSet`.
35. M0 HF8 FAIL->PASS Closure: identischer HF8-Testsatz ist pre-fix FAIL und post-fix PASS dokumentiert inkl. all-board matrix.
36. M1 Settings IA Split: klare Subtab-Gruppierung reduziert Such- und Wechselzeit in Settings.
37. M2 Quick-Mode Engine: gemeinsamer Moduszustand (`off`/`activate`/`deactivate`/`clear`) mit sichtbarem Guard.
38. M3 Activation Sprint Flow: eine gewaehlte Animation wird per Room-Click sequentiell auf viele Raeume angewendet.
39. M4 Deactivation Sprint Flow: eine gewaehlte Animation wird per Room-Click sequentiell aus vielen Raeumen entfernt.
...
- Multi-play-area und single-area Boards liefern im selben Lifecycle dieselbe canonical apply Semantik ohne area-drop drift.
- Firefox headless diagnostics und Chrome/mobile-class parity traces zeigen identische source-merge/fallback verdicts.
