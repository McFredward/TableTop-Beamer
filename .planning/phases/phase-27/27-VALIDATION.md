...
| B7 | Right-click on inner intersection shows two delete options | Menu | **Pre-condition:** grid has at least one inner row AND at least one inner column (i.e. >3 in each axis); on a fresh 3×3 default the deletion guards refuse, so first right-click an empty area and use "Add horizontal line here" once (yielding a 4×3 grid). Then right-click ≤ 10 px from the inner intersection in row 2 → menu shows "Delete vertical line" (hidden if outer column) + "Delete horizontal line" + a SINGLE "Add line through this point" item (whose action adds BOTH a perpendicular horizontal AND a perpendicular vertical line at the click coordinate, per locked D-10 + 27-UI-SPEC). | ⬜ pending |
