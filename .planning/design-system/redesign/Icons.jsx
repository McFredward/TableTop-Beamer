// Icon primitives — minimal, stroke-based, Lucide-aligned outlines.
// Every icon is 24x24 viewbox, 1.75 stroke, rounded caps/joins.
// No dependencies — inline SVG only.
...
function Icon({ name, size = 20, strokeWidth = 1.75, filled = false, style }) {
  const d = ICON_DEFS[name];
  if (!d) return <span style={{ width: size, height: size, display: "inline-block", outline: "1px dashed red", fontSize: 9 }}>{name}?</span>;
  return (
