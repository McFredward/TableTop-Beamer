...
- Current Phase: — (51 closed 2026-05-22, shipped as v1.1.1)
...
- Previous Phase: 51 (CLOSED — 2026-05-22, Released as v1.1.1, Animation Name input keystroke focus loss: Phase 49 gap-closure-25 added a defensive document.activeElement.blur() inside syncDirtyBar() to dismiss the mobile soft keyboard when the dirty bar appears. The blur fired on EVERY syncDirtyBar call where dirty=true, including the ~one per keystroke during a Name edit. Fixed by gating the blur to the false→true transition only via a new module-level _lastDirtyState tracking variable.)
- Earlier Phase: 50 (CLOSED — 2026-05-21, Released as v1.1.0, Aspect-ratio-aware board import: stage now derives aspect-ratio at runtime from the loaded board image's naturalWidth/naturalHeight via new --board-aspect / --board-aspect-num CSS custom properties.)
