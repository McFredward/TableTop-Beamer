...
- Current Phase: — (50 closed 2026-05-21, shipped as v1.1.0)
...
- Previous Phase: 50 (CLOSED — 2026-05-21, Released as v1.1.0, Aspect-ratio-aware board import: stage now derives aspect-ratio at runtime from the loaded board image's naturalWidth/naturalHeight via new --board-aspect / --board-aspect-num CSS custom properties. Replaces 3 hardcoded `7978/5456` CSS references; defaults preserved so existing Nemesis boards work unchanged. Listener bound on the boardImage's `load` event handles both boot and every board switch. Polygons remain 0..1 normalized so they scale automatically — no coordinate migration needed. Verified via Playwright: frostpunk 1172x1080 ~1.085:1 now renders at 1.0852:1 vs hardcoded 1.4623:1 pre-fix.)
